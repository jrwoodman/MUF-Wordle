...
: init-answer-words ( dbref -- )
    ...
    over "_words/answers/2" setpropstr
    ...
    over "_words/answers/3" setpropstr
    ...
    swap "_words/answers/9" setpropstr
;

: do-init ( str -- )
    strip
    
    ( Get the database object dbref )
    dup not if
        pop
        "Usage: wordle-db-init <dbref>" tell
        "Initializes word lists on the specified database object." tell
        exit
    then
    
    ( Convert string to dbref )
    atoi dbref
    
    ( Verify it's a valid object )
    dup ok? not if
        pop
        "Error: Invalid database object reference." tell
        exit
    then
    ...
    dup controls not if
        pop
        "Error: You don't have permission to modify that object." tell
        exit
    then
    
    "Initializing Wordle word database..." tell
    "" tell
    
    ( Store answer words )
    "Storing answer words..." tell
    dup init-answer-words
    
    ( Mark database as initialized )
    "initialized" over "_words/status" setpropstr
    systime over "_words/init_time" setprop
    me@ intostr over "_words/init_by" setpropstr
    
    pop
    
    "" tell
    "Database initialization complete!" tell
    "The word lists are now stored as properties." tell
    "Answer words: _words/answers/1 through _words/answers/9" tell
;

: cmd-wordle-db-init
    command @ "Wordle-DB-Init" setname
    command @ "Initialize Wordle word database" setdesc
